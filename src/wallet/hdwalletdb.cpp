...
{
    return WriteIC(std::make_pair(DBKeys::RHOM_SXADDR, sxAddr.scan_pubkey), sxAddr, true);
};

bool CHDWalletDB::ReadStealthAddress(CStealthAddress& sxAddr)
{
    // Set scan_pubkey before reading
    return m_batch.Read(std::make_pair(DBKeys::RHOM_SXADDR, sxAddr.scan_pubkey), sxAddr);
};

bool CHDWalletDB::EraseStealthAddress(const CStealthAddress& sxAddr)
{
    return EraseIC(std::make_pair(DBKeys::RHOM_SXADDR, sxAddr.scan_pubkey));
};


bool CHDWalletDB::ReadNamedExtKeyId(const std::string &name, CKeyID &identifier, uint32_t nFlags)
{
    return m_batch.Read(std::make_pair(DBKeys::RHOM_EXTKEYNAMED, name), identifier, nFlags);
};

bool CHDWalletDB::WriteNamedExtKeyId(const std::string &name, const CKeyID &identifier)
{
    return WriteIC(std::make_pair(DBKeys::RHOM_EXTKEYNAMED, name), identifier, true);
};


bool CHDWalletDB::ReadExtKey(const CKeyID &identifier, CStoredExtKey &ek32, uint32_t nFlags)
{
    return m_batch.Read(std::make_pair(DBKeys::RHOM_EXTKEY, identifier), ek32, nFlags);
};

bool CHDWalletDB::WriteExtKey(const CKeyID &identifier, const CStoredExtKey &ek32)
{
    return WriteIC(std::make_pair(DBKeys::RHOM_EXTKEY, identifier), ek32, true);
};


bool CHDWalletDB::ReadExtAccount(const CKeyID &identifier, CExtKeyAccount &ekAcc, uint32_t nFlags)
{
    return m_batch.Read(std::make_pair(DBKeys::RHOM_EXTACC, identifier), ekAcc, nFlags);
};

bool CHDWalletDB::WriteExtAccount(const CKeyID &identifier, const CExtKeyAccount &ekAcc)
{
    return WriteIC(std::make_pair(DBKeys::RHOM_EXTACC, identifier), ekAcc, true);
};
...
{
    return m_batch.Read(PackKey(DBKeys::RHOM_SXADDRKEYPACK, identifier, nPack), aksPak, nFlags);
};
...
{
    return WriteIC(PackKey(DBKeys::RHOM_SXADDRKEYPACK, identifier, nPack), aksPak, true);
};
...
{
    return m_batch.Read(std::make_pair(DBKeys::RHOM_FLAG, name), nValue, nFlags);
};

bool CHDWalletDB::WriteFlag(const std::string &name, int32_t nValue)
{
    return WriteIC(std::make_pair(DBKeys::RHOM_FLAG, name), nValue, true);
};
...
{
    return m_batch.Read(std::make_pair(DBKeys::RHOM_WALLETSETTING, setting), json, nFlags);
};

bool CHDWalletDB::WriteWalletSetting(const std::string &setting, const std::string &json)
{
    return WriteIC(std::make_pair(DBKeys::RHOM_WALLETSETTING, setting), json, true);
};

bool CHDWalletDB::EraseWalletSetting(const std::string &setting)
{
    return EraseIC(std::make_pair(DBKeys::RHOM_WALLETSETTING, setting));
};
