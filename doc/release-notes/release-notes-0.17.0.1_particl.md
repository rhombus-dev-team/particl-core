...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `rhombus.conf` file, not inside included files or from
  ...
  as rhombus.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the rhombus.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=rhombus
    test.uacomment=rhombus-testnet
...
be used in V0.17 by starting rhombusd with the '-deprecatedrpc=accounts'
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When rhombus is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If rhombus is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart rhombusd with
...
- When running rhombusd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If rhombusd
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI rhombus:// (krab)
...
- #13604 `75bea05` Add depends 32-bit arm support for rhombus-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling rhombus-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to rhombus.conf (MarcoFalke)
...
- #12882 `d96bdd7` Make `test_rhombus` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.rhombus is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_rhombus` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in rhombusd systemd service file (Flowdalic)
- #12652 `55f490a` rhombus-cli: Provide a better error message when rhombusd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to rhombusd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for rhombus address in translations (kaplanmaxe)
...
- #13603 `453ae5e` rhombus-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix rhombus-cli --version (Empact)
...
- #12877 `2b54155` Use rhombusd in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed rhombus-cli -help output for help2man (hebasto)
